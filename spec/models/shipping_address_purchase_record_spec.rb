...
RSpec.describe ShippingAddressPurchaseRecord, type: :model do
  before do
    @shipping_address_purchase_record = FactoryBot.build(:shipping_address_purchase_record)
  end
  ...
  describe '商品購入機能' do
    ...
    context '購入できないとき' do
      it '郵便番号の保存にはハイフンがなければ購入できない' do
        ...
        expect(@shipping_address_purchase_record.errors.full_messages).to include("Postal_code ")
      end
      it '電話番号は11桁以内の数値でなければ購入できない' do
        @shipping_address_purchase_record.tel = '123'
        @shipping_address_purchase_record.valid?
        expect(@shipping_address_purchase_record.errors.full_messages).to include("Tel ")
      end
    end
  end
end